...
listenerList.click = function (targetList,camera) {
	...
	function down(event) {
		...
		console.log("test" + camera.isPerspectiveCamera);
		Mouse.setFromCamera(new THREE.Vector2(( event.clientX / canvas.width ) * 2 - 1,- ( event.clientY / canvas.height ) * 2 + 1), camera);
	    list = getObjList(targetList);
